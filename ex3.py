...
matrix_file = "mat.npy" if len(sys.argv) < 4 else sys.argv[1]
...
for t in range(1, time_slots):
    for char_index in range(len(expected_with_blanks)):
        ...
        if expected_with_blanks[char_index] == blank or (
                char_index > 1 and expected_with_blanks[char_index] == expected_with_blanks[char_index - 2]):
            cal_matrix[char_index, t] = (get_val(cal_matrix, char_index - 1, t - 1) + get_val(cal_matrix, char_index,
                                                                                              t - 1)) * \
                                        probability[expected_with_blanks[char_index]][t]
        else:
            cal_matrix[char_index, t] = (get_val(cal_matrix, char_index - 2, t - 1) + get_val(cal_matrix,
                                                                                              char_index - 1,
                                                                                              t - 1) + get_val(
                cal_matrix, char_index, t - 1)) * probability[expected_with_blanks[char_index]][t]
